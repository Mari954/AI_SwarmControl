...
//MSP432 Equivalent to above which is done in the RSLK setup function
...
void setup() 
{
  ...
  rosNode.initNode();                            //init ROS node
  rosNode.getHardware()->setBaud(57600);         //set baud for ROS serial communication
  rosNode.subscribe(cmd_vel);                    //suscribe to ROS topic for velocity commands
  rosNode.advertise(speed_pub);                  //prepare to publish speed in ROS topic

  setupRSLK();                              //initialization needed for the RSLK.
...
void loop() 
{
  //Check ROS Node from raspberry pi
  rosNode.spinOnce();
  if((millis()-lastMilli) >= LOOPTIME)   
  {                                                                           // enter timed loop
    lastMilli = millis();
    
    if (!rosNode.connected()){
      analogWrite(RED_LED, lightValueNoComm[lightInc]);
      lightInc=lightInc+1;
      if (lightInc >= 25)
      {
        lightInc=0;
    ...
    {
      analogWrite(RED_LED, lightValue [lightInc]);
    ...
    {
      setMotorSpeed(0,abs(PWM_leftMotor));
    ...
    {
      setMotorSpeed(0,abs(PWM_rightMotor));
...
{
  speed_msg.header.stamp = rosNode.now();      //timestamp for odometry data
  speed_msg.vector.x = speed_act_left;    //left wheel speed (in m/s)
  speed_msg.vector.y = speed_act_right;   //right wheel speed (in m/s)
  speed_msg.vector.z = time/1000;         //looptime, should be the same as specified in LOOPTIME (in s)
  speed_pub.publish(&speed_msg);
  rosNode.spinOnce();
  rosNode.loginfo("Publishing odometry");
}

//Left motor encoder counter
void encoderLeftMotor() 
{
  if (digitalRead(ENCODER_ELA_PIN) == digitalRead(ENCODER_ELB_PIN)) pos_left++;
  else pos_left--;
}
...
{
  if (digitalRead(ENCODER_ERA_PIN) == digitalRead(ENCODER_ERB_PIN)) pos_right--;
  else pos_right++;
}
